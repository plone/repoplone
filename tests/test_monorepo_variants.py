...
def test_frontend_only_versions_next(test_frontend_only_project, bust_path_cache, runner):
    result = runner.invoke(app, ["versions", "next"])
    assert result.exit_code == 0
...
def test_backend_only_deps_upgrade_frontend_error(test_backend_only_project, bust_path_cache, runner):
    result = runner.invoke(app, ["deps", "upgrade", "frontend", "1.0.0"])
    assert result.exit_code == 1
    assert "Error: Frontend component is not enabled" in result.stdout

def test_frontend_only_deps_upgrade_backend_error(test_frontend_only_project, bust_path_cache, runner):
    result = runner.invoke(app, ["deps", "upgrade", "backend", "1.0.0"])
    assert result.exit_code == 1
    assert "Error: Backend component is not enabled" in result.stdout

def test_backend_only_no_section_settings(test_backend_only_no_section_project, bust_path_cache):
    result = settings.get_settings()
    assert result.backend.enabled is True
    assert result.frontend.enabled is False
    assert result.sanity() is True
...
def test_backend_only_no_section_versions_next(test_backend_only_no_section_project, bust_path_cache, runner):
    result = runner.invoke(app, ["versions", "next"])
    assert result.exit_code == 0
    assert "Backend" in result.stdout
    assert "Frontend" not in result.stdout

def test_frontend_only_no_section_settings(test_frontend_only_no_section_project, bust_path_cache):
    result = settings.get_settings()
    assert result.backend.enabled is False
    assert result.frontend.enabled is True
    assert result.sanity() is True
...
def test_frontend_only_no_section_versions_next(test_frontend_only_no_section_project, bust_path_cache, runner):
    result = runner.invoke(app, ["versions", "next"])
    assert result.exit_code == 0
    assert "Frontend" in result.stdout
    assert "Backend" not in result.stdout
